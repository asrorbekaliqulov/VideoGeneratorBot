...
async def start_remove_admin(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    await msg.edit_message_text("Iltimos, adminlikdan o'chirmoqchi bo'lgan foydalanuvchini tanlang:", reply_markup=reply_markup)
    return SELECT_ADMIN
...
async def select_admin(update: Update, context: CallbackContext) -> int:
    ...
    if userID == user_id:
        await context.bot.send_message(chat_id=userID, text="Siz o'zingizni adminlikdan o'chira olmaysiz!")
        return SELECT_ADMIN
...
async def confirm_remove(update: Update, context: CallbackContext) -> int:
    ...
    if user:
        await query.edit_message_text(f"{user.first_name} @{user.username} adminlikdan muvaffaqiyatli o'chirildi.")
