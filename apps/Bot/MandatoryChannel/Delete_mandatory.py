...
def Delete_channels(channel_id):
    return Channel.objects.filter(channel_id=channel_id).delete()  # QuerySet ni ro'yxatga aylantirish
...
async def start_delete_mandatory(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for channel in channels:
        keyboards.append([InlineKeyboardButton(text=f"{channel.name}", callback_data=f"xDeleted_{channel.channel_id}")])
