...
async def Check_bot_administrator(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if update.message.chat_shared is None:
        await update.message.reply_text("<b>Menimcha siz pastdagi tugmani bosmadingiz😤</b>\nIltimos quyidagi tugmalarni bosing", reply_markup=reply_markup, parse_mode="HTML")
        return "START_ADD_CHANNEL"
    ...
    if is_saved:
        await update.message.reply_text("<b>Kanal/Guruh allaqachon mavjud!</b>", parse_mode="HTML")
    ...
    try:
        ...
        if channel_save:
            await update.message.reply_text("<b>Muvaffaqiyatli saqlandi!!!</b>", parse_mode="HTML")
            return ConversationHandler.END
    except:
        await update.message.reply_text("Xatolik ro'y berdi menimcha botni admin qilmadingiz.")
        return "START_ADD_CHANNEL"
